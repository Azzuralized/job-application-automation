...
def extract_core_position(position: str) -> str:
    """
    Extract core position name without employment type suffixes.
    Example:
        "Education Counselor (Full Time)" → "education counselor"
        "Software Engineer (Remote)" → "software engineer"
    """
    if not position:
        return ""
    
    # Remove common employment type suffixes in parentheses
    core = re.sub(
        r"\s*\((full[- ]time|part[- ]time|remote|hybrid|onsite|contract|permanent|temporary|freelance|internship)\)\s*",
        "",
        position,
        flags=re.IGNORECASE
    ).strip()
...
def validate_identity(
    ...
    # --------------------------------------------------------
    # Position validation (more lenient)
    # --------------------------------------------------------
    if position:
        # Try exact match first
        if not contains_text(combined_text, position):
            # Try core position without employment type
            core_position = extract_core_position(position)
            if core_position and not contains_text(combined_text, core_position):
                errors.append(
                    f"Posisi '{position}' "
                    "tidak ditemukan dalam generated content."
                )
